...
function LiveMatchScreen({ navigation, route }) {
  ...
  function handleEventsData(data, firebaseKey, matchMode) {
    const events = [];
    let time = null;
  ...
  useEffect(() => {
    const sendSeconds = async () => {
      ...
      if (!timeEntryKey) {
        const response = await postData(tournamentName, data, urlPath);

        setTimeEntryKey(response.data.name);
      } else {
        updateData(tournamentName, data, urlPath, timeEntryKey);
      }
    };
