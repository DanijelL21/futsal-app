...
function GamesScreen({ navigation, route }) {
  ...
  const teamsNr = competitionInfo.teamsNr;
  const isAdmin = authCtx.isAuthenticated(competitionName);

  useEffect(() => {
    if (competitionInfo.mode == "tournaments") {
      setCompetitionPhase("Group Stage");
    } else {
      setCompetitionPhase("Round 1");
    }
  ...
  useFocusEffect(
    ...
    useCallback(() => {
      async function fetchGames() {
        try {
          if (!competitionPhase) {
            // Avoid fetching games if competitionPhase isn't set yet
            return;
          }
  ...
  return (
    <Background>
      <GrupsDropdownMenu setSelectedPhase={setCompetitionPhase} />
      {gameList.length === 0 ? (
        <>
